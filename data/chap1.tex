...
\fbox{概念}\hspace{0.5em} 对于函数$y=f(x)$,若$\int_{0}^{\infty}f\left(x\right)e^{-sx}dx$收敛，则称此为对$f(x)$的拉普拉斯变换，记为$L(y)$或$L(f(x))$，$s$为参数
...
\fbox{例题}\hspace{0.5em}(2021·张宇八套卷) 若$\int_{0}^{\infty}e^{-ax}\cdot cos\left(bx\right)dx$收敛，则$a、b$的范围\underline{\hbox to 15mm{}}

\fbox{解析}\hspace{0.5em} 由上述弹簧振子的讨论，保证$e^{-ax}$为衰减函数即可，即$a>0,b$任意
...
\fbox{结论}\hspace{1em} $L\left(y^{\left(n\right)}\right)=s^{n}\cdot L\left(y\right)-s^{n-1}\cdot y\left(0\right)-s^{n-2}\cdot y^{\prime}\left(0\right)-......-y^{(n-1)}\left(0\right)$
 ...
 \vspace{-40pt}
 	...
 	\begin{align*}
 		% 第1行：带证明框的公式，对齐点设置在第二个等号前
 		\text{\fbox{证明}} \hspace{2em} L\left(y'(x)\right) &= \int_{0}^{\infty} y'(x) \cdot e^{-sx} dx = \int_{0}^{\infty} e^{-sx} d\left(y(x)\right) \\[5pt]
 		% 第2行：分部积分结果，=号与第1行的对齐点自动对齐
 		&= \left. e^{-sx} \cdot y(x) \right|_{0}^{\infty} + s \cdot \int_{0}^{\infty} y(x) e^{-sx} dx \\[-5pt]
 		% 第3行：化简后的最终结果，=号与前两行对齐（最后一行不加\\）
 		&= sL(y) - y(0)
 	\end{align*}
...
\fbox{题目} \hspace{0.5em} (2020·数一)设$f(x)$满足在$f^{\prime\prime}\left(x\right)+af^{\prime}\left(x\right)+f\left(x\right)=0\:\left(a>0\right)$

\hspace{3.5em}$f\left(0\right)=m\:,\:f^{\prime}\left(0\right)=n\:,则\:\int_{0}^{\infty}f\left(x\right)dx=$\underline{\hbox to 20mm{}}
...
\fbox{题目} \hspace{0.2em} (2016·数一)设函数$y(x)$满足方程$y^{\prime\prime}+2y^{\prime}+ky=0$,其中$0<k<1$.
...
\fbox{解析} \hspace{0.2em} $(1)$ 设$y\left(0\right)=C_{1}\:,\:y^{\prime}\left(0\right)=C_{2}$,对方程取拉式变换，应用其微分性质
...
\begin{center}
	$L\left(xy\right)=-\dfrac{d}{ds}\left\lbrack L\left(y\right)\right\rbrack\:,\:L\left(xy^{\prime}\right)=-\dfrac{d}{ds}\left\lbrack L\left(y^{\prime}\right)\right\rbrack\:$
\end{center}

通常这种变系数的微分方程，可采用如下三种方式较为合适
...
\begin{spacing}{2.2}
	\begin{center}
		\begin{tabular}{ccc}
			\textbullet \hspace{0.5em} 欧拉公式  &  \textbullet \hspace{0.5em} 分离变量再积分  & \textbullet \hspace{0.5em} 分项组合凑微分 
		\end{tabular} 
		
		\vspace{5pt}
		
		$\left(\dfrac{x}{y}\right)^{\prime}=\dfrac{y-xy^{\prime}}{y^{2}}$ ，\hspace{2em}
		$\left(\dfrac{y}{x}\right)^{\prime}=\dfrac{y^{\prime}x-y}{x^{2}}$ ，\hspace{2em}
		$\left(xy\right)^{\prime}=y+xy^{\prime}$
...
\begin{center}
	$L\left\lbrack\int_{0}^{x}y\left(t\right)dt\right\rbrack=\dfrac{L\left(y\right)}{s}$  \hspace{2em}
	详情见后续章节-卷积公式
	
	\vspace{10pt}
	
	还有像函数的积分性质、延迟定理，不必掌握
\end{center}
